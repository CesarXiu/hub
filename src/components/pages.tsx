...
export default function Pages({ paginas }: PagesProps) {
  const NEXT_PUBLIC_IP_SERVER = process.env.NEXT_PUBLIC_IP_SERVER;
        ...
        {paginatedPages[currentPage].map((pagina: Pagina, index: number) => {
          return (
            <li
              ...
              onClick={() =>
                handleClick(pagina.type + "://" + NEXT_PUBLIC_IP_SERVER + "/" + pagina.url)
              }
            >
              <div className="grid grid-flow-col justify-between gap-4">
                <div className="justify-between overflow-hidden" id="info">
